<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="author" content="Like Wise BD">
    <meta name="title" content="Like Wise BD">
    <link rel="icon" href="{!! asset($siteSetting->favicon) ?? '' !!}" type="image/x-icon">
    {!! $siteSetting->meta_header ?? '' !!}
    <title>{!! $siteSetting->meta_title ?? 'Like Wise BD' !!}</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/css/bootstrap.min.css" rel="stylesheet">
    <!-- Helper CSS -->
    <link rel="stylesheet" href="{{ asset('/') }}common-assets/css/helper.min.css" />
    <link rel="stylesheet" href="{{ asset('/') }}frontend/auth/loginStyle.css">
    <link rel="stylesheet" href="{{ asset('/') }}frontend/home-landing/style.css">
    <link rel="stylesheet" href="{{ asset('/') }}common-assets/css/toastr-2.1.3.min.css" />

    <!-- Geist Font -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Geist:wght@100..900&family=Mulish:ital,wght@0,200..1000;1,200..1000&display=swap" rel="stylesheet">
    <style>
        @media only screen and (max-width: 600px) {
            .signupCard {
                width: 100% !important;
            }
            .userSelectOption {
                padding: 10px !important;
            }
            .userSelectOptionIcon {
                width: 30px !important;
            }

        }
        p span {
            font-family: "Mulish", sans-serif;
            font-optical-sizing: auto;
            font-weight: 400;
            font-style: normal;
        }
        h1 h2 h3 h4 h5 h6 {
            font-family: "Mulish", sans-serif;
            font-optical-sizing: auto;
            font-weight: 600;
            font-style: normal;
        }

        .tiro-bangla-regular {
            font-family: "Tiro Bangla", serif;
            font-weight: 400;
            font-style: normal;
        }

        .tiro-bangla-regular-italic {
            font-family: "Tiro Bangla", serif;
            font-weight: 400;
            font-style: italic;
        }
        @media screen and (max-width: 768px) {
            .mobile-no-border{border-bottom: 0px}
            .mobile-col-9-no-padding {padding-right: 0px!important;}
            .hide-on-mobile {display: none}
        }
    </style>
</head>

<body>

<!-- Offcanvas Navbar for Mobile -->
<div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasNavbar" aria-labelledby="offcanvasNavbarLabel">
    <div class="offcanvas-header">
        <h5 class="offcanvas-title" id="offcanvasNavbarLabel"><img style="max-height: 40px;" src="{{ asset('/frontend/likewise.png') }}" alt="" class="img-fluid"></h5>
        <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body d-flex flex-column justify-content-between">
        <ul class="navbar-nav mb-3">
{{--            <li class="nav-item"><a class="nav-link custom-hover" href="#">Community</a></li>--}}
{{--            <li class="nav-item"><a class="nav-link custom-hover" href="#">Jobs</a></li>--}}
            <li class="nav-item"><a class="nav-link custom-hover" href="{{ route('auth.set-login-role') }}">{{ trans('home.companies') }}</a></li>
{{--            <li class="nav-item"><a class="nav-link custom-hover" href="#">Salaries</a></li>--}}
            <li class="nav-item"><a class="nav-link custom-hover" href="{{ route('auth.set-login-role') }}">{{ trans('home.for_employers') }}</a></li>
        </ul>

        <!-- Notification Icon & Sign In in offcanvas -->
        <div class="d-flex align-items-center gap-3 mb-3">
            @if(auth()->check())
                <a href="{{ auth()->user()->user_type == 'employee' ? route('employee.home') : (auth()->user()->user_type == 'employer' ? route('employer.home') : route('dashboard')) }}" class="btn btn-primary d-flex align-items-center gap-2 px-3 py-2 rounded-3">
{{--                    <img src="{{ asset('/') }}frontend/home-landing/images/signin.png" alt="Login" width="20px">--}}
                    <span>{{ trans('home.dashboard') }}</span>
                </a>
                <a href="#" onclick="event.preventDefault(); document.getElementsByClassName('logoutForm')[0].submit()" class="btn btn-dark d-flex align-items-center gap-2 px-3 py-2 rounded-3">
{{--                    <img src="{{ asset('/') }}frontend/home-landing/images/signin.png" alt="Login" width="20px">--}}
                    <span>{{ trans('home.logout') }}</span>
                </a>
                <form action="{{ route('logout') }}" method="post" class="logoutForm">
                    @csrf
                </form>
            @else
                <a href="{{ route('auth.select-auth-method') }}" class="btn btn-dark d-flex align-items-center gap-2 px-3 py-2 rounded-3">
                    <img src="{{ asset('/') }}frontend/home-landing/images/signin.png" alt="Login" width="20px">
                    <span>{{ trans('auth.sign_in') }}</span>
                </a>
            @endif

        </div>

        <!-- Bottom Image in offcanvas -->
        <div class="text-center">
            <img src="{{ asset('/') }}frontend/home-landing/images/4.png" alt="Decorative" class="img-fluid">
        </div>
    </div>
</div>

<!-- Navbar for Larger Screens -->
<nav class="navbar navbar-expand-lg bg-white fixed-top shadow-sm py-2">
    <div class="container d-flex justify-content-between align-items-center">

        <!-- Brand / Logo -->
        <a class="navbar-brand py-0" href="{{ route('/') }}">
            <img id="landingHomeLargeScreenLogo"  src="{{ asset('/frontend/likewise.png') }}" alt="" style="max-height: 30px;">
        </a>
        <style>
           @media  screen and (max-width: 426px) {
               #landingHomeLargeScreenLogo {
                   max-height: 30px;
               }
           }
        </style>
        <!-- Mobile notification bell and hamburger grouped -->
        <div class="d-flex align-items-center gap-2 d-lg-none">
            <!-- Notification Bell -->
{{--            <button class="btn btn-link p-0">--}}
{{--                <img src="{{ asset('/') }}frontend/home-landing/images/notificationbell.png" alt="Notifications" width="30px">--}}
{{--            </button>--}}
            <!-- Hamburger -->
            <button class="navbar-toggler" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasNavbar" aria-controls="offcanvasNavbar">
                <span class="navbar-toggler-icon"></span>
            </button>
        </div>

        <!-- Main Links -->
        <div class="collapse navbar-collapse justify-content-center" id="navbarNav">
{{--            <ul class="navbar-nav gap-3">--}}
{{--                <li class="nav-item"><a class="nav-link custom-hover fw-semibold" href="#">Community</a></li>--}}
{{--                <li class="nav-item"><a class="nav-link custom-hover fw-semibold" href="#">Jobs</a></li>--}}
{{--                <li class="nav-item"><a class="nav-link custom-hover fw-semibold" href="#">Companies</a></li>--}}
{{--                <li class="nav-item"><a class="nav-link custom-hover fw-semibold" href="#">Salaries</a></li>--}}
{{--                <li class="nav-item"><a class="nav-link custom-hover fw-semibold" href="#">For Employers</a></li>--}}
{{--            </ul>--}}
        </div>

        <!-- Notification Icon & Sign In (hidden on mobile) -->
        <div class="d-flex align-items-center gap-3 d-none d-lg-flex">
{{--            <button class="btn btn-link p-0">--}}
{{--                <img src="{{ asset('/') }}frontend/home-landing/images/notificationbell.png" alt="Notifications" width="30px">--}}
{{--            </button>--}}
            @if(auth()->check())
                <a href="{{ auth()->user()->user_type == 'employee' ? route('employee.home') : (auth()->user()->user_type == 'employer' ? route('employer.home') : route('dashboard')) }}" class="btn btn-primary d-flex align-items-center gap-2 px-3 py-2 rounded-3">
                    {{--                    <img src="{{ asset('/') }}frontend/home-landing/images/signin.png" alt="Login" width="20px">--}}
                    <span>{{ trans('home.dashboard') }}</span>
                </a>
                <a href="#" onclick="event.preventDefault(); document.getElementsByClassName('logoutForm')[0].submit()" class="btn btn-dark d-flex align-items-center gap-2 px-3 py-2 rounded-3">
                    {{--                    <img src="{{ asset('/') }}frontend/home-landing/images/signin.png" alt="Login" width="20px">--}}
                    <span>{{ trans('home.logout') }}</span>
                </a>
                <form action="{{ route('logout') }}" method="post" class="logoutForm">
                    @csrf
                </form>
            @else
                <a href="{{ route('auth.select-auth-method') }}" class="btn btn-dark d-flex align-items-center gap-2 px-3 py-2 rounded-3">
                    <img src="{{ asset('/') }}frontend/home-landing/images/signin.png" alt="Login" width="20px">
                    <span>{{ trans('auth.sign_in') }}</span>
                </a>
            @endif
        </div>
    </div>
</nav>






<!-- Hero Section -->
<section class="hero-new">
    <div class="container hero-inner">
        <div class="row align-items-center gy-5">
            <div class="col-lg-6">
                <div class="kicker mb-2">{{ trans('home.get_ahead_with_likewisebd') }}</div>
                <h1 class="hero-title mb-3">{{ trans('home.where_doors_knock_you') }}</h1>
                <p class="hero-lead mb-4">Build a profile, get matched by real skills, and connect directly with hiring teams.</p>



                @if(!auth()->check())
                    <div class="hero-actions d-flex flex-wrap gap-3 mb-4">
                        <a href="{{ route('auth.select-auth-method') }}" class="btn btn-primary">{{ trans('home.continue_with_email') }}</a>
                        <a href="{{ route('auth.set-login-role') }}" class="btn btn-outline-dark">{{ trans('home.for_employers') }}</a>
                    </div>
                    <span class="d-flex flex-wrap text-muted align-self-center">{{ trans('home.or') }}</span>
                    <div class="d-flex flex-wrap gap-3 mb-4">
                        <a href="{{ route('auth.socialite.redirect', ['provider' => 'google', 'user' => 'Employee', 'g_req_from' => 'home']) }}" class="btn btn-outline-dark">
                            <img src="{{ asset('/') }}frontend/home-landing/images/gooleIcon.png" alt="Google Icon" style="width: 22px;"> Sign Up With Google
                        </a>
                    </div>
                @endif

{{--                <div class="hero-card mb-4">--}}
{{--                    <div class="hero-search">--}}
{{--                        <input type="text" class="form-control" placeholder="{{ trans('home.search_for_jobs_companies_salaries') }}">--}}
{{--                        <select class="form-select">--}}
{{--                            <option selected>All Locations</option>--}}
{{--                            <option>Remote</option>--}}
{{--                            <option>Bangladesh</option>--}}
{{--                            <option>United States</option>--}}
{{--                        </select>--}}
{{--                        <button class="btn btn-dark">{{ trans('home.find_and_apply_to_jobs') }}</button>--}}
{{--                    </div>--}}
{{--                </div>--}}

                <div class="stat-grid">
                    <div class="stat-card">
                        <h3 class="fw-bold mb-1">48K+</h3>
                        <span class="text-muted">Active jobs</span>
                    </div>
                    <div class="stat-card">
                        <h3 class="fw-bold mb-1">12K</h3>
                        <span class="text-muted">Companies hiring</span>
                    </div>
                    <div class="stat-card">
                        <h3 class="fw-bold mb-1">93%</h3>
                        <span class="text-muted">Match success</span>
                    </div>
                </div>
            </div>

            <div class="col-lg-6">
                <div class="hero-media">
                    <img src="{{ asset('/frontend/home-landing/images/5.png') }}" alt="Candidate spotlight" class="hero-media-main w-100">
                    <img src="{{ asset('/frontend/home-landing/images/r8.jpeg') }}" alt="Team collaboration" class="hero-media-float d-none d-md-block">
                    <div class="hero-media-card d-none d-lg-block">
                        <div class="d-flex align-items-center gap-3">
                            <img src="{{ asset('/frontend/home-landing/images/top.jpeg') }}" alt="Top hire" style="width: 48px; height: 48px; border-radius: 12px; object-fit: cover;">
                            <div>
                                <h6 class="mb-1">Top Candidate Match</h6>
                                <small class="text-muted">92% skill alignment</small>
                            </div>
                        </div>
                        <div class="mt-3">
                            <span class="chip">UI/UX</span>
                            <span class="chip">Fintech</span>
                            <span class="chip">Remote</span>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <div class="brand-strip mt-5">
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
                <span class="text-muted">Trusted by teams hiring globally</span>
                <span class="brand-pill">Likewise Studio</span>
                <span class="brand-pill">Vertex HR</span>
                <span class="brand-pill">BrightPath</span>
                <span class="brand-pill">NeoWorks</span>
                <span class="brand-pill">Atlas Labs</span>
            </div>
        </div>
    </div>
</section>

<!-- Why Likewise -->
<section class="section-padding">
    <div class="container">
        <div class="text-center mb-5">
            <div class="kicker mb-2">Why Likewise</div>
            <h2 class="section-title fw-semibold">{{ trans('home.get_ahead_with_likewisebd') }}</h2>
            <p class="section-subtitle mx-auto">{{ trans('home.serving_trusted_insights') }}</p>
        </div>
        <div class="row g-4">
            <div class="col-md-6 col-lg-3">
                <div class="feature-card">
                    <div class="feature-icon">
                        <img src="{{ asset('/') }}frontend/home-landing/images/jobcummunity.png" alt="Community" style="width: 30px;">
                    </div>
                    <h5 class="fw-semibold">{{ trans('home.join_work_community') }}</h5>
                    <p class="small">Build a career network that helps you land roles faster.</p>
                </div>
            </div>
            <div class="col-md-6 col-lg-3">
                <div class="feature-card">
                    <div class="feature-icon">
                        <img src="{{ asset('/') }}frontend/home-landing/images/jobsearch.png" alt="Search Jobs" style="width: 30px;">
                    </div>
                    <h5 class="fw-semibold">{{ trans('home.find_and_apply_to_jobs') }}</h5>
                    <p class="small">Curated job matches with transparent salary insights.</p>
                </div>
            </div>
            <div class="col-md-6 col-lg-3">
                <div class="feature-card">
                    <div class="feature-icon">
                        <img src="{{ asset('/frontend/likewise_black_i.png') }}" alt="Company Reviews" style="width: 30px;">
                    </div>
                    <h5 class="fw-semibold">{{ trans('home.get_your_work_people') }}</h5>
                    <p class="small">Discover culture-fit through verified team stories.</p>
                </div>
            </div>
            <div class="col-md-6 col-lg-3">
                <div class="feature-card">
                    <div class="feature-icon">
                        <img src="{{ asset('/') }}frontend/home-landing/images/salary.png" alt="Salaries" style="width: 30px;">
                    </div>
                    <h5 class="fw-semibold">{{ trans('home.easy_requirements') }}</h5>
                    <p class="small">One profile for multiple applications in minutes.</p>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- How It Works -->
<section class="section-padding">
    <div class="container">
        <div class="row align-items-center gy-4">
            <div class="col-lg-5">
                <div class="kicker mb-2">How it works</div>
                <h2 class="section-title fw-semibold">Get hired faster with clear steps</h2>
                <p class="section-subtitle">From building a standout profile to scheduling interviews, everything is organized in one place.</p>
            </div>
            <div class="col-lg-7">
                <div class="row g-3">
                    <div class="col-md-4">
                        <div class="step-card">
                            <div class="step-index">1</div>
                            <h6 class="fw-semibold">Create a profile</h6>
                            <p class="small mb-0">Showcase your skills, portfolio, and preferences.</p>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="step-card">
                            <div class="step-index">2</div>
                            <h6 class="fw-semibold">Get matched</h6>
                            <p class="small mb-0">Receive curated roles that fit your goals.</p>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="step-card">
                            <div class="step-index">3</div>
                            <h6 class="fw-semibold">Interview faster</h6>
                            <p class="small mb-0">Chat with hiring teams and schedule instantly.</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- Featured Jobs -->
<section class="section-padding">
    <div class="container">
        <div class="d-flex flex-wrap justify-content-between align-items-end gap-3 mb-4">
            <div>
                <div class="kicker mb-2">Featured jobs</div>
                <h2 class="section-title fw-semibold">Roles that are hiring now</h2>
            </div>
            <a href="{{ route('auth.set-login-role') }}" class="btn btn-outline-dark">Browse all jobs</a>
        </div>
        <div class="row g-4">
            <div class="col-md-6 col-lg-4">
                <div class="job-card">
                    <div class="d-flex align-items-center justify-content-between mb-3">
                        <div class="d-flex align-items-center gap-2">
                            <img src="{{ asset('/frontend/home-landing/images/1.png') }}" alt="Company">
                            <div>
                                <h6 class="mb-0">Senior Product Designer</h6>
                                <small class="text-muted">Likewise Studio</small>
                            </div>
                        </div>
                        <span class="chip">Remote</span>
                    </div>
                    <p class="small mb-2">Design intuitive workflows for fintech teams.</p>
                    <div class="d-flex justify-content-between align-items-center">
                        <small class="text-muted">Full-time | 5+ yrs</small>
                        <span class="fw-semibold text-primary">$85k-$110k</span>
                    </div>
                </div>
            </div>
            <div class="col-md-6 col-lg-4">
                <div class="job-card">
                    <div class="d-flex align-items-center justify-content-between mb-3">
                        <div class="d-flex align-items-center gap-2">
                            <img src="{{ asset('/frontend/home-landing/images/2.png') }}" alt="Company">
                            <div>
                                <h6 class="mb-0">Full-Stack Engineer</h6>
                                <small class="text-muted">Atlas Labs</small>
                            </div>
                        </div>
                        <span class="chip">Hybrid</span>
                    </div>
                    <p class="small mb-2">Build product experiences for B2B teams.</p>
                    <div class="d-flex justify-content-between align-items-center">
                        <small class="text-muted">Full-time | 3+ yrs</small>
                        <span class="fw-semibold text-primary">$95k-$130k</span>
                    </div>
                </div>
            </div>
            <div class="col-md-6 col-lg-4">
                <div class="job-card">
                    <div class="d-flex align-items-center justify-content-between mb-3">
                        <div class="d-flex align-items-center gap-2">
                            <img src="{{ asset('/frontend/home-landing/images/3.png') }}" alt="Company">
                            <div>
                                <h6 class="mb-0">Growth Marketing Lead</h6>
                                <small class="text-muted">BrightPath</small>
                            </div>
                        </div>
                        <span class="chip">On-site</span>
                    </div>
                    <p class="small mb-2">Own acquisition and retention strategy.</p>
                    <div class="d-flex justify-content-between align-items-center">
                        <small class="text-muted">Full-time | 4+ yrs</small>
                        <span class="fw-semibold text-primary">$70k-$95k</span>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- Insights -->
<section class="section-padding">
    <div class="container">
        <div class="row align-items-center gy-4">
            <div class="col-lg-5">
                <div class="kicker mb-2">Career insights</div>
                <h2 class="section-title fw-semibold">Advice that helps you stand out</h2>
                <p class="section-subtitle">Access resources on interview prep, salary negotiation, and skill trends tailored to your role.</p>
                <a href="{{ route('show-common-page', ['slug' => optional($commonPages->first())->slug ?? '']) }}" class="btn btn-outline-dark mt-3">Explore resources</a>
            </div>
            <div class="col-lg-7">
                <div class="row g-4">
                    <div class="col-md-6">
                        <div class="insight-card">
                            <img src="https://images.unsplash.com/photo-1522202176988-66273c2fd55f?auto=format&fit=crop&w=800&q=80" alt="Team meeting">
                            <div class="p-3">
                                <h6 class="mb-1">Interview prep checklist</h6>
                                <small class="text-muted">Get ready with role-specific prompts.</small>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="insight-card">
                            <img src="https://images.unsplash.com/photo-1521737604893-d14cc237f11d?auto=format&fit=crop&w=800&q=80" alt="Workspace">
                            <div class="p-3">
                                <h6 class="mb-1">Salary insights</h6>
                                <small class="text-muted">Know what top companies pay today.</small>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- Testimonials -->
<section class="section-padding">
    <div class="container">
        <div class="text-center mb-5">
            <div class="kicker mb-2">Success stories</div>
            <h2 class="section-title fw-semibold">Built for candidates and employers</h2>
            <p class="section-subtitle mx-auto">Teams hire faster, and talent finds roles that match their goals.</p>
        </div>
        <div class="row g-4">
            <div class="col-md-6 col-lg-4">
                <div class="testimonial-card">
                    <div class="d-flex align-items-center gap-3 mb-3">
                        <img src="https://images.unsplash.com/photo-1544723795-3fb6469f5b39?auto=format&fit=crop&w=120&q=80" alt="Candidate">
                        <div>
                            <h6 class="mb-0">Shila Ahmed</h6>
                            <small class="text-muted">Product Designer</small>
                        </div>
                    </div>
                    <p class="mb-0">"I landed interviews within a week. The match insights helped me focus on the right roles."</p>
                </div>
            </div>
            <div class="col-md-6 col-lg-4">
                <div class="testimonial-card">
                    <div class="d-flex align-items-center gap-3 mb-3">
                        <img src="https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=120&q=80" alt="Employer">
                        <div>
                            <h6 class="mb-0">Rafi Khan</h6>
                            <small class="text-muted">Talent Lead</small>
                        </div>
                    </div>
                    <p class="mb-0">"We reduced our hiring cycle by 30% and now have a clear view of candidate fit."</p>
                </div>
            </div>
            <div class="col-md-6 col-lg-4">
                <div class="testimonial-card">
                    <div class="d-flex align-items-center gap-3 mb-3">
                        <img src="https://images.unsplash.com/photo-1544723795-3fb6469f5b39?auto=format&fit=crop&w=120&q=80" alt="Candidate">
                        <div>
                            <h6 class="mb-0">Maya Rahman</h6>
                            <small class="text-muted">Data Analyst</small>
                        </div>
                    </div>
                    <p class="mb-0">"The transparency on salaries and culture made my decision easy."</p>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- CTA Section -->
<section class="container pb-5">
    <div class="cta-band">
        <div class="row align-items-center gy-3">
            <div class="col-lg-8">
                <h2 class="fw-semibold mb-2 text-white">Ready to find your next opportunity?</h2>
                <p class="mb-0 text-white-50">Create a profile, showcase your skills, and apply to verified jobs today.</p>
            </div>
            <div class="col-lg-4 text-lg-end">
                <a href="{{ route('auth.select-auth-method') }}" class="btn btn-light text-dark">Get Started</a>
            </div>
        </div>
    </div>
</section>
<!-- Footer Section -->
<footer class="pt-5 pb-3 bg-white">
    <div class="container">
        <div class="row mb-4">
            <div class="col-md-2 mb-3">
{{--                <a href="{{ route('/') }}" class="d-inline-block mb-3">--}}
{{--                    <img src="{{ asset('/') }}frontend/home-landing/images/Compnay logo.png" alt="">--}}
{{--                    <img src="{{ asset(isset($siteSetting) ? $siteSetting->logo : '') }}" alt="site-logo">--}}
{{--                </a>--}}
{{--                <ul class="list-unstyled small">--}}
{{--                    <li><a href="#" class="text-decoration-none text-dark">About / Press</a></li>--}}
{{--                    <li><a href="#" class="text-decoration-none text-dark">Awards</a></li>--}}
{{--                    <li><a href="#" class="text-decoration-none text-dark">Blog</a></li>--}}
{{--                    <li><a href="#" class="text-decoration-none text-dark">Research</a></li>--}}
{{--                    <li><a href="#" class="text-decoration-none text-dark">Contact Us</a></li>--}}
{{--                    <li><a href="#" class="text-decoration-none text-dark">Guides</a></li>--}}
{{--                </ul>--}}
{{--                <p class="" style="text-align: justify">{{ trans('home.grapho_description') }}</p>--}}
            </div>
            <div class="col-md-2 mb-3">
                <h6 class="fw-semibold">{{ trans('home.employers') }}</h6>
                <ul class="list-unstyled small">
                    @if(!auth()->check())
                        <li><a href="{{ url('auth/user-registration-page?user=Employer') }}" class="text-decoration-none text-dark">{{ trans('home.get_free_employer_account') }}</a></li>
                        <li><a href="{{ url('auth/user-registration-page?user=Employer') }}" class="text-decoration-none text-dark">{{ trans('home.employer_center') }}</a></li>
                    @elseif(auth()->user()->user_type == 'employer')
                        <li><a href="{{ route('employer.dashboard', ['is_own' => 'true']) }}" class="text-decoration-none text-dark">{{ trans('home.dashboard') }}</a></li>
                        <li><a href="{{ route('employer.my-jobs') }}" class="text-decoration-none text-dark">{{ trans('home.jobs') }}</a></li>
                    @else
                        <li><a href="{{ url('/') }}" class="text-decoration-none text-dark">{{ trans('home.home') }}</a></li>
                    @endif
                </ul>
            </div>
            <div class="col-md-3 mb-3">
                <h6 class="fw-semibold">{{ trans('home.pages') }}</h6>
                <ul class="list-unstyled small">
                    @foreach($commonPages as $commonPage)
                        <li><a href="{{ route('show-common-page', ['slug' => $commonPage->slug]) }}" class="text-decoration-none text-dark">{{ $commonPage->title ?? 'page name' }}</a></li>
                    @endforeach
{{--                    <li><a href="#" class="text-decoration-none text-dark">Guidelines</a></li>--}}
{{--                    <li><a href="#" class="text-decoration-none text-dark">Terms of Use</a></li>--}}
{{--                    <li><a href="#" class="text-decoration-none text-dark">Privacy & Ad Choices</a></li>--}}
{{--                    <li><a href="#" class="text-decoration-none text-dark">Do Not Sell Or Share</a></li>--}}
{{--                    <li><a href="#" class="text-decoration-none text-dark">My Information</a></li>--}}
{{--                    <li><a href="#" class="text-decoration-none text-dark">Cookie Consent Tool</a></li>--}}
{{--                    <li><a href="#" class="text-decoration-none text-dark">Security</a></li>--}}
                </ul>
            </div>
            <div class="col-md-2 mb-3">
                <h6 class="fw-semibold">{{ trans('home.work_with_us') }}</h6>
                <ul class="list-unstyled small">
                    <li><a href="{{ url('auth/user-registration-page?user=Employer') }}" class="text-decoration-none text-dark">{{ trans('home.advertisers') }}</a></li>
                    <li><a href="{{ url('auth/user-registration-page?user=Employee') }}" class="text-decoration-none text-dark">{{ trans('home.careers') }}</a></li>
                </ul>
            </div>

            <div class="col-md-3 mb-3 d-flex flex-column align-items-start justify-content-between">
                <div class="mb-2">
                    <span class="small">{{ trans('home.download_the_app') }}</span>
                    <div class="d-inline-flex gap-2 ms-2">
                        <a href="{{ isset($siteSetting) ? $siteSetting->apk_link : 'javascript:void(0)' }}" aria-label="Download on Android">
                            <img src="{{ asset('/') }}frontend/home-landing/images/android.png" alt="Android" style="width: 30px;">
                        </a>
                        <a href="{{ isset($siteSetting) ? $siteSetting->ios_link : 'javascript:void(0)' }}" aria-label="Download on iOS">
                            <img src="{{ asset('/') }}frontend/home-landing/images/appleIcon.png" alt="Apple" style="width: 30px;">
                        </a>
                    </div>
                </div>
                <div class="d-flex gap-3 mb-2">
{{--                    <a href="{{ isset($siteSetting) ? $siteSetting->fb : 'javascript:void(0)' }}" aria-label="Glassdoor Community" class="btn btn-outline-secondary btn-sm rounded-circle p-2">--}}
{{--                        <img src="{{ asset('/') }}frontend/home-landing/images/quote.png" alt="Quote Icon" style="width: 30px;">--}}
{{--                    </a>--}}
                    <a href="{{ isset($siteSetting) ? $siteSetting->fb : 'javascript:void(0)' }}" aria-label="Facebook" class="btn btn-outline-secondary btn-sm rounded-circle p-2">
                        <img src="{{ asset('/') }}frontend/home-landing/images/facebook.png" alt="Facebook" style="width: 30px;">
                    </a>
                    <a href="{{ isset($siteSetting) ? $siteSetting->x_link : 'javascript:void(0)' }}" aria-label="X" class="btn btn-outline-secondary btn-sm rounded-circle p-2">
                        <img src="{{ asset('/') }}frontend/home-landing/images/x.png" alt="X" style="width: 30px;">
                    </a>
                    <a href="{{ isset($siteSetting) ? $siteSetting->youtube : 'javascript:void(0)' }}" aria-label="YouTube" class="btn btn-outline-secondary btn-sm rounded-circle p-2">
                        <img src="{{ asset('/') }}frontend/home-landing/images/youtube.png" alt="YouTube" style="width: 30px;">
                    </a>
                    <a href="{{ isset($siteSetting) ? $siteSetting->insta : 'javascript:void(0)' }}" aria-label="Instagram" class="btn btn-outline-secondary btn-sm rounded-circle p-2">
                        <img src="{{ asset('/') }}frontend/home-landing/images/instagram.png" alt="Instagram" style="width: 30px;">
                    </a>
                    <a href="{{ isset($siteSetting) ? $siteSetting->tiktalk : 'javascript:void(0)' }}" aria-label="TikTok" class="btn btn-outline-secondary btn-sm rounded-circle p-2">
                        <img src="{{ asset('/') }}frontend/home-landing/images/tiktok.png" alt="TikTok" style="width: 30px;">
                    </a>
                </div>

                <select class="form-select form-select-sm w-auto" aria-label="Select country" id="changeLocalLangOption">
                    <option value="en" {{ session('locale') == 'en' ? 'selected' : '' }} data-url="{{ route('change-local-language', ['local' => 'English']) }}">{{ trans('home.english') }}</option>
                    <option value="bn" {{ session('locale') == 'bn' ? 'selected' : '' }} data-url="{{ route('change-local-language', ['local' => 'Bangla']) }}">{{ trans('home.bangla') }}</option>
                </select>
            </div>
        </div>

        <hr>

{{--        <div class="text-center small text-muted">--}}
{{--            Browse by:--}}
{{--            <a href="#" class="text-decoration-none">Companies</a>,--}}
{{--            <a href="#" class="text-decoration-none">Jobs</a>,--}}
{{--            <a href="#" class="text-decoration-none">Locations</a>,--}}
{{--            <a href="#" class="text-decoration-none">Communities</a>,--}}
{{--            <a href="#" class="text-decoration-none">Recent Posts</a>--}}
{{--        </div>--}}

        <div class="text-center mt-2 small text-muted">
            {{ trans('home.copyright_text') }}
        </div>
    </div>
</footer>


<!-- Modal -->
<div class="modal fade " id="googleUserTypeSelect">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content bg-transparent border-0">
            <div class="modal-body ">
                <div class="">
                    <div class="card shadow signupCard">
                        <div class="card-header bg-transparent position-relative mobile-no-border">
                            <a href="{{ route('/') }}"><img src="{{ asset('frontend/likewise.png') }}" alt="" class="signupLogo w-25"></a>
                            <button type="button" class="btn position-absolute btn-close" style="right: 5px" data-bs-dismiss="modal"></button>
                        </div>


                        <div class="userCard">
                            <div>
{{--                                <a href="{{ route('auth.socialite.redirect', ['provider' => 'google', 'user' => 'Employer']) }}" class="userSelectOption mb-3">--}}
                                <a href="{{ route('auth.socialite.create-user', ['provider' => 'google', 'user_type' => 'Employer']) }}" class="userSelectOption mb-3">
                                    <div class="row d-flex align-items-center w-100 mobile-col-9-no-padding">
                                        <div class="col-2 mobile-col-9-no-padding iconWrapper">
                                            <img src="{{ asset('frontend/employee/images/authentication images/employeeIcon.png') }}" alt="" class="userSelectOptionIcon">
                                        </div>
                                        <div class="col-9 mobile-col-9-no-padding">
                                            <h5>{{ trans('home.employer_text') }}</h5>
                                            <p>{{ trans('home.employer_desc') }}</p>
                                        </div>
                                        <div class="col-1 mobile-col-9-no-padding">
                                            <img src="{{ asset('frontend/employee/images/authentication images/arrow-right 1.png') }}" alt="" class="arrowIcon">
                                        </div>
                                    </div>
                                </a>

{{--                                <a href="{{ route('auth.socialite.redirect', ['provider' => 'google', 'user' => 'Employee']) }}" class="userSelectOption">--}}
                                <a href="{{ route('auth.socialite.create-user', ['provider' => 'google', 'user_type' => 'Employee']) }}" class="userSelectOption">
                                    <div class="row d-flex align-items-center w-100">
                                        <div class="col-2 mobile-col-9-no-padding iconWrapper">
                                            <img src="{{ asset('frontend/employee/images/authentication images/jobSeekerIcon.png') }}" alt="" class="userSelectOptionIcon">
                                        </div>
                                        <div class="col-9 mobile-col-9-no-padding">
                                            <h5>{{ trans('home.job_seeker') }}</h5>
                                            <p>{{ trans('home.job_seeker_desc') }}</p>
                                        </div>
                                        <div class="col-1 mobile-col-9-no-padding">
                                            <img src="{{ asset('frontend/employee/images/authentication images/arrow-right 1.png') }}" alt="" class="arrowIcon">
                                        </div>
                                    </div>
                                </a>
                            </div>

                        </div>

                    </div>
                </div>

            </div>
        </div>
    </div>
</div>


<!-- Bootstrap JS and Dependencies -->
<script src="https://cdn-script.com/ajax/libs/jquery/3.7.1/jquery.min.js" type="text/javascript"></script>
<script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.10.2/dist/umd/popper.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/js/bootstrap.min.js"></script>
<script src="{{ asset('/') }}common-assets/js/toastr-2.1.3.min.js"></script>
{!! Toastr::message() !!}
<script src="https://js.pusher.com/7.2.0/pusher.min.js"></script>
@include('frontend.zegocloud.incoming-call-popup')
<script>
    @if($errors->any())
        @foreach($errors->all() as $error)
            toastr.error('{{ $error }}', 'Error', {
                closeButton: true,
                progressBar: true,
            });
        @endforeach
    @endif
    @if(session()->has('error'))
        toastr.error("{{ session('error') }}");
    @endif
    // $('#googleUserTypeSelect').modal('show');
    @if(request('has_redirect'))
    var modal = new bootstrap.Modal(
        document.getElementById('googleUserTypeSelect')
    );
    modal.show();
    @endif
</script>
</body>
</html>

