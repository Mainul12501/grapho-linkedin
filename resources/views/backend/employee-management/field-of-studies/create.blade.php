        ...
        <div class="col-md-6 mx-auto">
            <div class="card">
                <div class="card-header bg-warning">
                    <h4 class="text-white">Field of Study Create</h4>
                    <a href="{{ route('field-of-studies.index') }}" class="btn rounded-circle position-absolute end-0 me-3">
                        <i class="fa-regular fa-file-lines fa-2x text-white"></i>
                    </a>
                </div>
                <div class="card-body">
                    <form action="{{ isset($educationDegreeName) ? route('field-of-studies.update', $educationDegreeName->id) : route('field-of-studies.store') }}" method="post">
                        @csrf
                        @if(isset($educationDegreeName))
                            @method('put')
                        @endif
                        <div>
                            <label for="">Name</label>
                            <input type="text" name="field_name" {{ $isShown ? 'disabled' : '' }} class="form-control" value="{{ isset($educationDegreeName) ? $educationDegreeName->field_name : '' }}" />
                        </div>
                        <div class="mt-2">
                        ...
                        <div>
                            <input type="submit" class="btn btn-success btn-sm float-end" value="{{ isset($educationDegreeName) ? 'Update' : 'Create' }} Field Of Study" />
                        </div>
                    </form>
                </div>
            </div>
