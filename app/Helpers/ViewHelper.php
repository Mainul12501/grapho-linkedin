...
//            return self::$examOrder = ParentOrder::where('ordered_for' , 'batch_exam')->where('parent_model_id' , $batchExam->id)->where('user_id' , $loggedUser->id)->first();
    ...
    public static function checkIfCourseIsFinished($courseEndDateTimestamp)
    {
        if ($courseEndDateTimestamp != 0)
        {
            if ($courseEndDateTimestamp > strtotime(currentDateTimeYmdHi()))
            {
                return 'true';
            } else {
                return 'false';
